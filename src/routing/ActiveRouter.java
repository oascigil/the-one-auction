...
public abstract class ActiveRouter extends MessageRouter {
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		//System.out.println(SimClock.getTime() +" Message received at host "+this.getHost()+" with id "+m.getId());
		if (recvCheck != RCV_OK) {
			return recvCheck;
		}
	...
	protected List<Tuple<Message, Connection>> getMessagesForConnected() {
		...
		for (Message m : getMessageCollection()) {
			for (Connection con : getConnections()) {
				...
				if (m.getTo() == to) {
					//System.out.println(getHost()+ " send message "+m.getId()+" type "+m.getProperty("type")+" "+m.getTo());
